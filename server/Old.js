...
app.use(
  cors({
    origin: ["https://rhythmandlyricsproject.com"],
...
const vidstorage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, '../public_html/uploads/videos')
  },
  filename: function (req, file, cb) {
    cb(null, Date.now() + file.originalname)
  }
...
let transporter = nodemailer.createTransport({
  ...
  const mailOptions = {
    ...
    <div></div>
    <h1><strong> Verify Your Account</strong></h1>
    <h2>Welcome to <strong>Rhythm and Lyrics Project</strong>, ${stagename}</h2>
    <p> You are Just one Step away, we need to confirm your email address </p>
    ...
    href="https://rhythmandlyricsproject.com/contestants.email.verification/${verificationToken}"> Comfirm Email Address 
    </a>
    <br><br>
    </div>
    </div>
          ...
          if (result.length > 0) {
              bcrypt.compare(password, result[0].password, (error, response) => {
                  if (response) {
                    ...
                    res.send({ message: result[0].email })
                  } else {
...
app.get('/api/loggedin/:id', (req, res) => {
